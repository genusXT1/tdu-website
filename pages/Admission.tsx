          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-[#003366] p-10 rounded-[3rem] shadow-2xl text-white relative overflow-hidden">
              ...
              <ul className="space-y-4 text-white/70 text-sm font-light leading-relaxed">
                <li className="flex gap-3"><div className="w-1.5 h-1.5 rounded-full bg-[#FFCC00] mt-2 flex-shrink-0"></div> {lang === 'BG' ? 'Диплома за средно образование (оригинал и превод)' : lang === 'RU' ? 'Аттестат о среднем образовании (оригинал и перевод)' : lang === 'RO' ? 'Diploma de bacalaureat (original și traducere)' : 'High school diploma (original and translation)'}</li>
                <li className="flex gap-3"><div className="w-1.5 h-1.5 rounded-full bg-[#FFCC00] mt-2 flex-shrink-0"></div> {lang === 'BG' ? 'Удостоверение за раждане' : lang === 'RU' ? 'Свидетельство о рождении' : lang === 'RO' ? 'Certificat de naștere' : 'Birth certificate'}</li>
                <li className="flex gap-3"><div className="w-1.5 h-1.5 rounded-full bg-[#FFCC00] mt-2 flex-shrink-0"></div> {lang === 'BG' ? 'Документ за българска народност' : lang === 'RU' ? 'Документ о болгарском происхождении' : lang === 'RO' ? 'Document de origine bulgară' : 'Document of Bulgarian origin'}</li>
                <li className="flex gap-3"><div className="w-1.5 h-1.5 rounded-full bg-[#FFCC00] mt-2 flex-shrink-0"></div> {lang === 'BG' ? 'Медицинско свидетелство' : lang === 'RU' ? 'Медицинская справка' : lang === 'RO' ? 'Certificat medical' : 'Medical certificate'}</li>
              </ul>
              <a
                href="https://apply.mon.bg/"
                target="_blank"
                rel="noreferrer"
                className="w-full mt-12 py-5 px-6 bg-[#FFCC00] text-[#003366] rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-white transition-all shadow-lg flex items-center justify-between gap-3 group"
              >
                <span className="flex-1 text-center leading-snug">{t.applyNow}</span>
            ...
            <div className="bg-[#FFCC00] p-10 rounded-[3rem] shadow-xl text-[#003366]">
              ...
              <h3 className="text-xl font-black uppercase tracking-tighter mb-4">
                {lang === 'BG' ? 'Важни дати' : lang === 'RU' ? 'Важные даты' : lang === 'RO' ? 'Date Importante' : 'Important Dates'}
              </h3>
              <div className="space-y-4 mb-8">
                {[
                  { date: '01.06 - 30.06', event: lang === 'BG' ? 'Подаване на документи' : lang === 'RU' ? 'Подача документов' : lang === 'RO' ? 'Depunerea actelor' : 'Submission of documents' },
                  { date: '05.07 - 10.07', event: lang === 'BG' ? 'Приемни изпити' : lang === 'RU' ? 'Вступительные экзамены' : lang === 'RO' ? 'Examene de admitere' : 'Entrance exams' },
                  { date: '15.07', event: lang === 'BG' ? 'Първо класиране' : lang === 'RU' ? 'Первый этап зачисления' : lang === 'RO' ? 'Primul clasament' : 'First ranking' },
                  { date: '20.07', event: lang === 'BG' ? 'Записване' : lang === 'RU' ? 'Зачисление' : lang === 'RO' ? 'Înscriere' : 'Enrollment' }
                ].map((d, i) => (
                  <div key={i} className="flex justify-between items-center border-b border-[#003366]/10 pb-2">
                    ...
                    <span className="text-xs font-medium text-right ml-2">{d.event}</span>
                  </div>
                ))}
              </div>
              <p className="text-[10px] font-medium leading-relaxed opacity-80">
                {lang === 'BG'
                  ...
                  : lang === 'RU'
                    ...
                    : lang === 'RO'
                      ? 'Urmăriți programul oficial al MON pentru eventuale modificări.'
                      : 'Follow the official MON schedule for any potential changes.'}
              </p>
            </div>
          </div>
