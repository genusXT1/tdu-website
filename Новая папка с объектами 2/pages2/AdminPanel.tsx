...
export const AdminPanel: React.FC = () => {
  ...
  const [galleryForm, setGalleryForm] = useState({ titleRU: '', category: 'Университет' });
  const [docForm, setDocForm] = useState({ titleRU: '', url: '', storagePath: '', category: 'study-plan' as any });
  
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: 'news' | 'partners' | 'pages' | 'gallery' | 'docs') => {
    const file = e.target.files?.[0];
    if (!file) return;
    
    const UPLOAD_SECRET = (import.meta as any).env.VITE_UPLOAD_SECRET || 'default-secret-change-me';

    if (target !== 'docs' && !file.type.startsWith('image/')) {
      showToast("Пожалуйста, выберите изображение", "error");
      return;
    }

    if (target === 'docs' && file.type !== 'application/pdf') {
    ...
    try {
      ...
      try {
        ...
        const storageRef = ref(storage, `uploads/${target}/${Date.now()}_${finalFileName}`);
      ...
      if (target === 'news') {
        ...
        const baseName = file.name.includes('.') ? file.name.split('.').slice(0, -1).join('.') : file.name;
        setDocForm(prev => ({ ...prev, url, titleRU: prev.titleRU || baseName }));
      } else if (target === 'gallery') {
        await addDoc(collection(db, "gallery"), {
          url,
          title: { RU: galleryForm.titleRU || finalFileName, BG: galleryForm.titleRU || finalFileName },
          category: galleryForm.category,
          timestamp: serverTimestamp()
        });
        setGalleryForm({ titleRU: '', category: 'Университет' });
      }
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, u => {
      setUser(u);
      if (u) {
        // Simple RBAC: check if email contains 'sysadmin' or is a specific one
        const isSys = u.email?.toLowerCase().includes('sysadmin') || u.email === 'admin@tdu-tr.md';
        ...
        onSnapshot(query(collection(db, "documents"), orderBy("timestamp", "desc")), (s) => {
  ...
  const syncTelegram = async (silent = false) => {
    ...
    try {
      ...
      if (!resData.ok) {
        if (!silent) alert(`Ошибка Telegram API: ${resData.description || 'Неизвестная ошибка'}`);
        return;
      }
      ...
      for (const update of updates) {
        ...
        if (msg.photo) {
          try {
            ...
            if (fData.ok) {
              const tgFileUrl = `https://api.telegram.org/file/bot${token}/${fData.result.file_path}`;
              
              // Download and upload to local server
              const imgBlob = await fetch(tgFileUrl).then(r => r.blob());
              const formData = new FormData();
              formData.append('file', imgBlob, `tg_${fileId}.jpg`);
              
              const uploadResp = await fetch('/api/upload', {
                method: 'POST',
                headers: { 'x-upload-auth': UPLOAD_SECRET },
                body: formData
              });
              ...
              if (uploadResp.ok) {
                ...
                // Fallback to direct TG link if upload fails
                imageUrls.push(tgFileUrl);
              }
            }
          } catch (imgErr) {
            console.error("Failed to process TG image:", imgErr);
          }
        }
      ...
      if (!silent && count > 0) showToast(`Успешно! Добавлено ${count} новых сообщений.`);
      else if (!silent && count === 0) showToast("Новых сообщений не найдено");
    } catch (e: any) { 
      console.error("TG Sync Error:", e);
      if (!silent) showToast("Ошибка синхронизации TG: " + e.message, "error");
  ...
  const handleSaveDoc = async () => {
    if (!docForm.titleRU || !docForm.url) {
    ...
    try {
      await addDoc(collection(db, "documents"), {
        title: { RU: docForm.titleRU, BG: docForm.titleRU, EN: docForm.titleRU, RO: docForm.titleRU },
        url: docForm.url,
        storagePath: docForm.storagePath,
        category: docForm.category,
        timestamp: serverTimestamp()
      });
      setDocForm({ titleRU: '', url: '', storagePath: '', category: 'study-plan' });
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col lg:flex-row font-sans relative">
      ...
      <aside className="lg:w-64 w-full bg-[#001a33] text-white p-2 lg:p-6 flex flex-row lg:flex-col sticky top-0 h-auto lg:h-screen z-[100] shadow-2xl lg:overflow-y-auto no-scrollbar gap-1 lg:gap-4 items-center lg:items-stretch flex-shrink-0">
        ...
        <nav className="flex flex-row lg:flex-col gap-1 shrink-0 lg:shrink w-full overflow-x-auto lg:overflow-x-visible no-scrollbar py-1">
          {Object.keys(TAB_INFO).filter(tabId => {
            if (role === 'sysadmin') return true;
            return !['logs', 'system'].includes(tabId);
          }).map(tabId => {
      ...
      <main className="flex-grow p-4 lg:p-12 overflow-y-auto">
        ...
        {role !== 'sysadmin' && ['logs', 'system'].includes(activeTab) && (
        ...
        {activeTab === 'docs' && (
          <div className="space-y-8 animate-fadeIn">
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase">Название документа (RU)</label>
                  <input placeholder="Учебный план 2025" className="w-full p-4 bg-slate-50 rounded-xl outline-none focus:ring-2 focus:ring-[#FFCC00]" value={docForm.titleRU} onChange={e => setDocForm({...docForm, titleRU: e.target.value})} />
                </div>
                <div className="space-y-2">
          ...
          <div className="space-y-8 animate-fadeIn">
            <div className="bg-white p-12 rounded-[3.5rem] border shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-300 ml-4">Название (RU)</label>
                  <input 
                    placeholder="Студенческая весна..." 
                    className="w-full p-4 bg-slate-50 rounded-2xl border-none outline-none focus:ring-2 focus:ring-[#FFCC00]" 
                    value={galleryForm.titleRU} 
                    onChange={e => setGalleryForm({...galleryForm, titleRU: e.target.value})} 
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-300 ml-4">Категория</label>
                  <select 
                    className="w-full p-4 bg-slate-50 rounded-2xl border-none outline-none focus:ring-2 focus:ring-[#FFCC00] font-bold text-[#003366]" 
                ...
                <label className="bg-[#003366] text-white px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest cursor-pointer hover:bg-[#FFCC00] hover:text-[#003366] transition-all shadow-lg text-center">
                  {uploading ? 'ЗАГРУЗКА...' : 'ВЫБРАТЬ И ЗАГРУЗИТЬ'}
