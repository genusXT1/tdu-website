    ...
    <div className="page-transition min-h-screen bg-[#fcfdfe]">
      ...
      <div className="max-w-6xl mx-auto px-6 pb-40 space-y-24">
        {isLoading ? (
          ...
          <>
            {groupedPrograms.pedagogy.length === 0 && groupedPrograms.other.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-slate-200">
                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs mb-4">
                  {lang === Language.RU ? 'Программы обучения пока не добавлены' : lang === Language.BG ? 'Все още няма добавени програми' : 'Nu există programe adăugate încă'}
                </p>
                <p className="text-slate-300 text-[10px] uppercase tracking-widest">
                  {lang === Language.RU ? 'Администратор может добавить их в панели управления' : lang === Language.BG ? 'Администраторът може да ги добави в контролния панел' : 'Administratorul le poate adăuga în panoul de control'}
                </p>
              </div>
            ) : (
              <>
