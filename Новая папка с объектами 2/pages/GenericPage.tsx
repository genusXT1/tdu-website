...
const BlockRenderer: React.FC<{
  ...
  switch (block.type) {
      ...
      return (
        <section className={`bg-[#001a33] text-white px-12 rounded-[4rem] relative overflow-hidden mb-16 ${wrapperStyle}`}>
          <div className="relative z-10 max-w-3xl text-left">
            ...
            >
              {t('content') || t('subtitle') || (isEditMode ? 'Hero content' : '')}
            </p>
          </div>
    ...
    case 'text':
      return (
        <div className={`px-6 md:px-12 mb-16 text-left ${wrapperStyle}`}>
          <div
            contentEditable={isEditMode}
            suppressContentEditableWarning
            onBlur={(e) =>
            ...
            className={`prose prose-slate max-w-4xl mx-auto text-lg leading-relaxed text-slate-700 font-light outline-none ${isEditMode ? 'focus:ring-2 focus:ring-[#FFCC00] rounded-lg p-2 -m-2' : ''}`}
          />
        </div>
      );
    ...
    case 'cta':
      return (
        <div className={`mb-24 ${wrapperStyle}`}>
          <div className="bg-[#FFCC00] rounded-[3rem] p-12 md:p-20 flex flex-col md:flex-row items-center justify-between gap-12 shadow-xl">
            <div className="max-w-2xl text-left">
              <h2
                ...
                className="text-4xl md:text-5xl font-serif italic text-[#003366] mb-6 outline-none"
              >
                {t('title') || 'CTA Title'}
              </h2>
              <p
                ...
                className="text-[#003366]/80 text-lg font-medium leading-relaxed outline-none"
              >
              ...
              {isEditMode ? (
                <div className="flex flex-col gap-2">
                  <input 
                    ...
                    value={data.buttonText?.[lang] || ''} 
                    onChange={e => updateBlock?.({...data, buttonText: {...data.buttonText, [lang]: e.target.value}})} 
                  />
                  <input 
                    type="text" 
                    placeholder="URL (например: /admission)" 
                    className="p-3 rounded-xl text-xs" 
                    value={data.buttonUrl || ''} 
                    onChange={e => updateBlock?.({...data, buttonUrl: e.target.value})} 
                  />
                </div>
              ) : (
                <Link 
                  to={data.buttonUrl || '#'} 
                  className="px-12 py-6 bg-[#003366] text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-white hover:text-[#003366] transition-all shadow-lg text-center whitespace-nowrap"
                >
                  {data.buttonText?.[lang] || 'Learn More'}
                </Link>
              )}
            </div>
          </div>
...
const GenericPage: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  // Default content for legal pages if they are empty
  if (!page && LEGAL_CONTENT[slug as keyof typeof LEGAL_CONTENT]) {
    const legal = LEGAL_CONTENT[slug as keyof typeof LEGAL_CONTENT];
    page = {
      id: `system-${slug}`,
      slug: slug,
      title: { BG: legal.BG.title, RU: legal.RU.title, RO: legal.BG.title, EN: legal.BG.title },
      blocks: [
        {
          id: 'hero-legal',
          type: 'hero',
        ...
        },
        {
          ...
          type: 'text',
          data: {
            content: { BG: legal.BG.content, RU: legal.RU.content, RO: legal.BG.content, EN: legal.BG.content }
          },
          style: { padding: 'py-20' }
        }
      ],
