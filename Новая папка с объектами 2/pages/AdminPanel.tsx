...
const ROLE_PERMISSIONS: Record<'admin' | 'sysadmin', string[]> = {
  ...
  sysadmin: ['dash', 'news', 'programs', 'docs', 'apps', 'partners', 'academic', 'gallery', 'pages', 'site', 'nav', 'logs', 'users', 'schedules', 'system'],
};
...
export const AdminPanel: React.FC = () => {
  ...
  const [userForm, setUserForm] = useState({ email: '', password: '', role: 'admin' as 'admin' | 'sysadmin', firstName: '', lastName: '' });
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, u => {
      ...
      if (u) {
        ...
        // Then try to get from Firestore for overrides
        getDocs(query(collection(db, "admin_users"), where("email", "==", u.email))).then(snap => {
          if (!snap.empty) {
  ...
  const generateSlug = (text: string) => {
    return text
      .toLowerCase()
      .replace(/[^\w\u0400-\u04FF\s-]/g, '') // Keep cyrillic, alphanumeric, spaces, hyphens
      .replace(/\s+/g, '-')
      .replace(/-+/g, '-')
      .trim();
  };
  ...
  const handleSaveNews = async () => {
    ...
    const payload = {
      ...
      excerpt: { 
        RU: newsForm.content.RU.substring(0, 150), 
        BG: newsForm.content.BG.substring(0, 150), 
        EN: newsForm.content.EN.substring(0, 150), 
        RO: newsForm.content.RO.substring(0, 150) 
      },
  ...
  const handleSaveUser = async () => {
    if (!userForm.email) return showToast("Введите email", 'error');
    setLoading(true);
    try {
      const q = query(collection(db, "admin_users"), where("email", "==", userForm.email));
      ...
      setUserForm({ email: '', password: '', role: 'admin', firstName: '', lastName: '' });
    } catch (e) {
      showToast("Ошибка сохранения", 'error');
    } finally {
      setLoading(false);
      ...
      <main className="flex-grow p-4 lg:p-12 overflow-y-auto">
        ...
        {activeTab === 'partners' && (
          <div className="space-y-8 animate-fadeIn">
            <div className="bg-white p-8 rounded-3xl border grid grid-cols-1 md:grid-cols-3 gap-6 shadow-xl border-slate-100">
              ...
              <button onClick={handleSavePartner} disabled={loading} className="bg-[#003366] text-white py-5 rounded-2xl col-span-1 md:col-span-3 font-black uppercase text-[10px] tracking-widest hover:bg-[#FFCC00] hover:text-[#003366] transition-all">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
              {partners.map(p => (
                <div key={p.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 text-center relative group shadow-sm">
                  ...
                  <p className="text-[10px] font-black uppercase text-[#003366] mb-1">{p.name}</p>
                  {p.link && <p className="text-[8px] text-slate-400 truncate px-2">{p.link}</p>}
                </div>
              ))}
            </div>
          </div>
              ...
              <div className="fixed inset-0 z-[1100] flex items-center justify-center p-0 sm:p-6 backdrop-blur-xl bg-[#000d1a]/80 animate-fadeIn">
                <div className="bg-white w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-4xl sm:rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col relative">
                  ...
                  <div className="p-8 overflow-y-auto space-y-8">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Текст новости ({adminLang})</label>
                      <textarea 
                        ...
                        onChange={e => setNewsForm({...newsForm, content: { ...newsForm.content, [adminLang]: e.target.value }})} 
                      />
                    </div>
                  </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <input placeholder="admin@example.com" className="w-full p-5 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-[#FFCC00]" value={userForm.email} onChange={e=>setUserForm({...userForm, email:e.target.value})} />
                </div>
                <div className="space-y-2">
          ...
          <div className="space-y-8 animate-fadeIn">
            <div className="bg-white p-12 rounded-[3.5rem] border shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-300 ml-4">Название ({adminLang})</label>
                  <input 
